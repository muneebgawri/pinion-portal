...
class ShortCode {
    public $per_page;
    public $settings;
    public $user_status_array = [ 'approved', 'pending', 'denied' ];
    /**
     * @var false|string
     */
    public $page_url;
     ...
     * @var mixed
     */
    public $roles_include;
    /**
     * @var array
     */
    public $status_exclude;
    /**
     * @var int
     */
    public $total;
    ...
    public function user_lists( $atts ) {
        ...
        if ( ! empty( $atts['status_exclude'] ) ) {
            $status_exclude    = array_map( 'trim', explode( ',', $atts['status_exclude'] ) );
        }

        $profile_base_value = ! empty( $_GET[ $profile_permalink_base ] ) ? wp_unslash( $_GET[ $profile_permalink_base ] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    ...
    /**
     * Get a url of supplied users profile
     *
     * @since 3.4.12
     *
     ...
     * @return string New URL query string (unescaped).
     */
    public function get_user_link_by_username( $user_name ) {
        return add_query_arg( [ 'username' => $user_name ], $this->page_url );
    }
