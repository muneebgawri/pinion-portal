...
namespace WeDevs\Wpuf\Pro\Admin\Forms\ProfileTemplates;
 ...
 */
class Dokan_Vendor_Reg_Template extends Form_Template {
    ...
    public function __construct() {
        ...
        $this->form_fields = [
            ...
            [
                ...
                'name'             => 'shopurl',
                'is_meta'          => 'yes',
                'help'             => function_exists( 'dokan_get_option' ) ? home_url() . '/' . dokan_get_option(
                        'custom_store_url', 'dokan_general', 'store'
                    ) . '/<strong id="url-alart"></strong><strong id="url-alart-mgs" class="pull-right"></strong>' : '',
                'width'            => 'large',
                'css'              => '',
                'placeholder'      => '',
                'default'          => '',
    ...
    public function handle_form_updates( $user_id, $form_id, $form_settings ) {
        ...
        $store_settings = [
            'store_name'   => isset( $_POST['dokan_store_name'] ) ? sanitize_text_field(
                wp_unslash( $_POST['dokan_store_name'] )
            ) : '',
            'phone'        => isset( $_POST['dokan_store_phone'] ) ? sanitize_text_field(
                wp_unslash( $_POST['dokan_store_phone'] )
            ) : '',
            'banner'       => isset( $_POST['wpuf_files']['dokan_banner'] ) ? absint(
                wp_unslash( $_POST['wpuf_files']['dokan_banner'][0] )
            ) : '',
            'gravatar'     => isset( $_POST['wpuf_files']['dokan_profile_picture'] ) ? absint(
                wp_unslash( $_POST['wpuf_files']['dokan_profile_picture'][0] )
            ) : '',
            'location'     => isset( $_POST['location'] ) ? sanitize_text_field(
                wp_unslash( $_POST['location'] )
            ) : '',
            'find_address' => isset( $_POST['find_address'] ) ? sanitize_text_field(
                wp_unslash( $_POST['find_address'] )
            ) : '',
            'address'      => isset( $_POST['dokan_address'] ) ? array_map(
                'sanitize_text_field', wp_unslash( $_POST['dokan_address'] )
            ) : '',
        ];
        ...
        $latitude  = ! empty( $store_settings['location'][0] ) ? $store_settings['location'][0] : 0;
        $longitude = ! empty( $store_settings['location'][1] ) ? $store_settings['location'][1] : 0;
        update_user_meta( $user_id, 'dokan_geo_latitude', $latitude );
        update_user_meta( $user_id, 'dokan_geo_longitude', $longitude );
        $store_settings['location']           = implode( ',', $store_settings['location'] );
        $store_settings['address']['country'] = ! empty( $store_settings['address']['country_select'] ) ? $store_settings['address']['country_select'] : '';
