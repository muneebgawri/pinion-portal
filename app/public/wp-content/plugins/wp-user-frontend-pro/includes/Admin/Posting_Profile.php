...
use WeDevs\Wpuf\Admin\Posting;
use WeDevs\Wpuf\WPUF_User;
use WP_User;

class Posting_Profile extends Posting {

    public function __construct() {
        add_action( 'personal_options_update', [ $this, 'save_fields' ] );
        add_action( 'edit_user_profile_update', [ $this, 'save_fields' ] );
        add_action( 'wpuf_pro_frontend_form_update_user_meta', [ $this, 'update_vendor_profile_meta' ], 10, 2 );
        add_action( 'show_user_profile', [ $this, 'render_form' ] );
        add_action( 'edit_user_profile', [ $this, 'render_form' ] );
        ...
        add_action( 'wpuf_load_post_forms', [ $this, 'user_profile_scripts' ] );
        add_action( 'wpuf_load_registration_forms', [ $this, 'user_profile_scripts' ] );
    }
    ...
    function render_form( $userdata, $post_id = NULL, $preview = false ) {
        $option = get_option( 'wpuf_profile', [] );
        if ( ! isset(
                $option['roles'][ $this->get_role_name(
                    $userdata
                ) ]
            ) || empty( $option['roles'][ $this->get_role_name( $userdata ) ] ) ) {
            return;
        }
        ...
        <input type="hidden" name="wpuf_cf_update"
        ...
        if ( is_admin() && current_user_can( 'edit_users' ) ) {
            ...
            <table class="form-table">
                <tr>
                    ...
                    <td>
                        ...
                        <span class="description"><?php _e(
                                'Lock user from creating new post.', 'wpuf-pro'
                            ); ?></span></em>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="text" name="wpuf_lock_cause" id="wpuf_lock_cause" class="regular-text"
                               value="<?php echo esc_attr( $lock_reason ); ?>"/>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="wpuf_edit_post_lock_cause" id="wpuf_edit_post_lock_cause"
                               class="regular-text" value="<?php echo esc_attr( $edit_lock_reason ); ?>"/>
                    </td>
                </tr>
            </table>
            <?php
    ...
    function post_lock_update( $user_id ) {
        ...
        $yes_no         = [ 'yes', 'no' ];
        $post_lock      = in_array( $_POST['wpuf_postlock'], $yes_no, true ) ? $_POST['wpuf_postlock'] : 'no';
        $edit_post_lock = in_array( $_POST['wpuf_edit_postlock'], $yes_no, true ) ? $_POST['wpuf_edit_postlock'] : 'no';

        update_user_meta( $user_id, 'wpuf_postlock', $post_lock );
        update_user_meta( $user_id, 'wpuf_edit_postlock', $edit_post_lock );

        if ( ! empty( $_POST['wpuf_lock_cause'] ) ) {
            update_user_meta( $user_id, 'wpuf_lock_cause', sanitize_text_field( $_POST['wpuf_lock_cause'] ) );
        }

        if ( ! empty( $_POST['wpuf_edit_post_lock_cause'] ) ) {
            update_user_meta(
                $user_id, 'wpuf_edit_post_lock_cause', sanitize_text_field( $_POST['wpuf_edit_post_lock_cause'] )
            );
        }
    }
