 ...
 * @since 2.5
 */
class List_Table_Profile_Forms extends \WP_List_Table {
    ...
    public function get_views() {
        ...
        $post_counts['publish'] = ! empty( $post_counts['publish'] ) ? $post_counts['publish'] : 0;
        $post_counts['trash']   = ! empty( $post_counts['trash'] ) ? $post_counts['trash'] : 0;
    ...
    public function get_bulk_actions() {
        ...
        if ( ! isset( $_GET['post_status'] ) || 'trash' !== $_GET['post_status'] ) {
            $actions['trash'] = __( 'Move to Trash', 'wpuf-pro' );
        }

        if ( isset( $_GET['post_status'] ) && 'trash' === $_GET['post_status'] ) {
            $actions['restore'] = __( 'Restore', 'wpuf-pro' );
            $actions['delete']  = __( 'Delete Permanently', 'wpuf-pro' );
        }
    ...
    public function prepare_items() {
        ...
        $this->counts = is_array( $items['count'] ) ? count( $items['count'] ) : $items['count'];
    ...
    public function get_columns() {
        $columns = array(
            ...
            'form_name' => __( 'Form Name', 'wpuf-pro' ),
            'user_role' => __( 'User Role', 'wpuf-pro' ),
            'shortcode' => __( 'Shortcode', 'wpuf-pro' ),
        );
    ...
    public function column_default( $item, $column_name ) {
        switch ( $column_name ) {
            ...
            case 'shortcode':
                return 'Registration: <code>[wpuf_profile type="registration" id="' . $item['ID'] . '"]</code><br /><br />' . 'Edit Profile: <code>[wpuf_profile type="profile" id="' . $item['ID'] . '"]</code>';
    ...
    public function column_form_name( $item ) {
        ...
        if ( ( !isset( $_GET['post_status'] ) || 'trash' !== $_GET['post_status'] ) && current_user_can( wpuf_admin_role() ) ) {
            $actions['edit']      = sprintf( '<a href="%s">%s</a>', $edit_url, __( 'Edit', 'wpuf-pro' ) );
            $actions['trash']     = sprintf( '<a href="%s" class="submitdelete">%s</a>', $trash_url, __( 'Trash', 'wpuf-pro' ) );
            $actions['duplicate'] = sprintf( '<a href="%s">%s</a>', $duplicate_url, __( 'Duplicate', 'wpuf-pro' ) );
        ...
        if ( ( isset( $_GET['post_status'] ) && 'trash' === $_GET['post_status'] ) && current_user_can( wpuf_admin_role() ) ) {
            $actions['restore'] = sprintf( '<a href="%s">%s</a>', $restore_url, __( 'Restore', 'wpuf-pro' ) );
            $actions['delete']  = sprintf( '<a href="%s" class="submitdelete">%s</a>', $delete_url, __( 'Delete Permanently', 'wpuf-pro' ) );
        ...
        $draft_marker = ( 'draft' === $item['post_status'] ) ?
                            '<strong> — <span class="post-state">' . __( 'Draft', 'wpuf-pro' ) . '</span></strong>' :
                            '';
