...
class Update {
    ...
    public function __construct() {
        if ( ! class_exists( 'WeDevs\Wpuf\Lib\Appsero\Client' ) ) {
            return;
        }
    ...
    protected function init_appsero() {
        $path = WPUF_PRO_INCLUDES . '/appsero_key.php';
        $code = '958afc63-99f8-4b98-b321-fcbc5cf95694';

        if ( file_exists( $path ) ) {
            $code = require $path;
        }

        $client = new \WeDevs\Wpuf\Lib\Appsero\Client( $code, __( 'WPUF Pro', 'wpuf-pro' ), WPUF_PRO_FILE );
...
//        if ( class_exists( '\Appsero\Updater' ) && function_exists( '\Appsero\Updater::init' ) ) {
    ...
    public function license_enter_notice() {
        ...
        <div class="notice error wpuf-license-notice">
            ...
            <div class="wpuf-license-notice__message">
                ...
                <p><?php printf( 'Please <a href="%s">enter</a> your valid <strong>WPUF Pro</strong> plugin license key to unlock more features, premium support and future updates.', admin_url( 'admin.php?page=wpuf_updates' ) ); ?></p>
            </div>
